...
import { Toast, Lazyload, Popup, Button, Tab, Tabs, Search, List, Field, Icon } from 'vant'

// @TIP 开发者自定义增加，自已使用到过的组件，不分别在各文件当中引用，避免每次引用的麻烦

Vue.use(Toast).use(Lazyload).use(Popup).use(Button).use(Tab).use(Tabs).use(Search).use(List).use(Field).use(Icon)
